...
extractCV.glm <- function(fit, scale, k, ...){
  fit.data <- eval(fit$call$data)
  fit.data <- fit.data[sample(nrow(fit.data)),]
  folds <- cut(seq(1, nrow(fit.data)), breaks=10, labels=FALSE)
  
  avg <- 0
  for(i in 1:10) {
      test.index <- which(folds==i, arr.ind=TRUE)
      test.data <- fit.data[test.index, ]
      train.data <- fit.data[-test.index, ]
      
      fit.glm <- glm(fit$call$formula, train.data, family=binomial)
      response <- predict(fit.glm, newdata=test.data)
      response <- 1 + as.integer(response > 0.5)
      actual <- factor(levels(fit$model), test.data$chd)
      predicted <- factor(levels(fit$model), response)
      e <- 1 - mean(sensitivity(predicted, actual), specificity(predicted, actual))
      avg <- avg + e
  }
  
  c(fit$model$rank, avg/10.)
}
```
